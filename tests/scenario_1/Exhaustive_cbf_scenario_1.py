...
for idx, comb in enumerate(all_comb):
    ...
    if len(x_r_list_comb) > 0:
        pred_frame = predictive_frame_lag(scenario_num, robot_type, x0, dt, tf, U_max, V_max=V_max, alpha_values=alpha_values, 
                                          beta_values=beta_values, num_constraints_hard=num_constraints_hard1, x_r_list=x_r_list_comb, 
                                          radius_list=radius_list_comb, reward_list=reward_list_comb, obstacle_list=obstacle_list, 
